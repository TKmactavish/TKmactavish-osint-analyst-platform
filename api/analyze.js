...
// ─── System prompt ────────────────────────────────────────────────────────────────────────────
const SYSTEM_PROMPT = 'You are a senior OSINT analyst for Athena Intel. Produce a structured intelligence brief as a single valid JSON object with NO markdown fences and NO text outside the JSON.\n\nGEOGRAPHIC SECURITY CONTEXT (apply when location matches):\n' + GEO_CONTEXT + '\nRULES:\n1. For location queries always apply geographic security context above.\n2. analytical_perspective: minimum 5 sentences covering situation assessment, patterns, source gaps, analyst flags, geopolitical context.\n3. recommendations: non-empty arrays (min 4 items each) for law_enforcement, private_sector, traveler.\n4. Return ONLY valid JSON, no markdown, no preamble.\n\nJSON SCHEMA:\n{"query":"string","type":"person|incident|location|organization|travel_risk","summary":"2-3 sentences","risk":"HIGH|MEDIUM|LOW","sources":[{"title":"string","url":"string","domain":"string","date":"YYYY-MM-DD or null","type":"official|mainstream|ngo|local|reference","confidence":"high|medium|low"}],"timeline":[{"date":"YYYY-MM-DD","event":"string","source_title":"string","source_url":"string","confidence":"high|medium|low"}],"flags":[{"name":"string","description":"string","severity":"high|medium|low","evidence":"string","source_url":"string or null"}],"risk_assessment":{"level":"HIGH|MEDIUM|LOW","rationale":"string","factors":["string"]},"analytical_perspective":"5+ sentences","recommendations":{"law_enforcement":["4+ items"],"private_sector":["4+ items"],"traveler":["4+ items"]}}';

// ─── Data fetchers ────────────────────────────────────────────────────────────────────────────
async function tFetch(url, opts, ms) {
  var ac = new AbortController();
  var tid = setTimeout(function() { ac.abort(); }, ms || 7000);
  return fetch(url, Object.assign({}, opts, { signal: ac.signal })).finally(function() {
    clearTimeout(tid);
  });
}
...
function parseRSS(xml) {
  ...
  while ((m = re.exec(xml)) !== null && out.length < 10) {
    ...
    if (clean && link) {
      var domain = '';
      try { domain = new URL(src[1] || '').hostname.replace(/^www\./, ''); } catch(e) {}
      var date = null;
      try { date = pub ? new Date(pub).toISOString().slice(0, 10) : null; } catch(e) {}
...
module.exports = async function handler(req, res) {
  ...
  try {
    var body = req.body || {};
    q = (body.query || '').trim();
  } catch(e) { return jsonRes({ error: 'Invalid JSON body' }, 400, res); }
  ...
  try {
    ...
    var result = await analyzeWithClaude(q, buildContext(results[0], results[1], results[2]), apiKey);
